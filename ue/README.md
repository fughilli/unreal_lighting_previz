...
For the corridor runner, use `configs/runner_1x7_20x20x20.json` (56,000 voxels,
same `previz_dev` shm): seven 20×20×20 modules laid end-to-end (effective z =
140, world `20x20x140`), one loopback controller each on ports 6454–6460 —
same modular pattern as artnet-tester's `sim_config_8.json`. Because the
modules stack along the buffer's z axis, the receiver's cube-concatenated
buffer is exactly the dense raster the UE component expects; lay the runner
horizontally by pitching the volume actor 90° (`VOLUME_PITCH` in
`scripts/setup_cesium_site.py`).
